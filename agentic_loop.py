...
PLAN = {
    "goal" : "Validate Student Enrolment App behavior using a local multi-agent worklow",
    "db_plan": [
...
def observe_subject_search(subject_code):
    ...
    for student in students:
        ...
        print(f"   Checked studnet_id={student[0]} -> {status}")
    ...
    return True, (
        f"Subject search validatin passed for {subject_code}"
    )
...
# === Model Call Helper ===
def call_model(model_name, system_prompt, user_prompt, max_tokens):
    try:
        client = OpenAI(
